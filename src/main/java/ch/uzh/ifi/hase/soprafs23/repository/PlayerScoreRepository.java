...
public interface PlayerScoreRepository extends JpaRepository<PlayerScore, Long> {
    ...
    @Query("SELECT NEW ch.uzh.ifi.hase.soprafs23.entity.PlayerStatistics(ps.player_id, COUNT(ps.id), MAX(ps.score) ,COUNT((CASE when ps.winner = true THEN 1 END))/COUNT((CASE WHEN ps.winner IS NOT NULL THEN 1 END))*1.0) FROM PlayerScore ps WHERE ps.player_id = :playerId GROUP BY ps.player_id")
